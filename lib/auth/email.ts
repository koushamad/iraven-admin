...
export async function sendApprovalEmail(opts: {
  ...
  const workspaceUrl = process.env.WORKSPACE_URL ?? 'http://localhost:3004'
  const loginUrl = `${workspaceUrl}/login`

  if (process.env.NODE_ENV !== 'production') {
    console.log(`\n[APPROVAL EMAIL → ${opts.to}] workspace: ${loginUrl}\n`)
  }

  await transporter.sendMail({
    from: `"IRaven Workspace" <${process.env.SMTP_FROM}>`,
    to: opts.to,
    subject: `Your IRaven Workspace is ready — ${opts.slug}.iraven.io`,
    text: `Hi ${opts.fullName},\n\nYour workspace request for ${opts.slug}.iraven.io has been approved.\n\nSign in at:\n${loginUrl}\n\nWelcome to IRaven.`,
    html: buildApprovalHtml(opts.fullName, opts.slug, loginUrl),
  })
}

function buildApprovalHtml(fullName: string, slug: string, loginUrl: string): string {
...
<head>
  ...
  <title>Your IRaven Workspace is ready</title>
  <style>
        ...
        <table width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:480px;">
          ...
          <tr>
            <td align="center" style="padding-bottom:36px;">
              <svg width="72" height="72" viewBox="0 0 430 430" xmlns="http://www.w3.org/2000/svg" style="display:block;margin:0 auto 16px;">
                ...
                <path fill="none" stroke="#08A88A" stroke-width="12" stroke-linecap="round" stroke-linejoin="round"
                  transform="translate(229.66 354.762)" d="M-18.232-17.262-32.16-2.924l22.5 23.162h39.32"/>
                <path fill="none" stroke="#08A88A" stroke-width="12" stroke-linecap="round" stroke-linejoin="round"
                  transform="translate(262.16 349.511)" d="m-2.16-20.12-22.5 22.805 22.5 22.804h24.32"/>
                ...
                <path fill="none" stroke="url(#rg)" stroke-width="12" stroke-linecap="round" stroke-linejoin="round"
                  ...
                  d="m80.676-26.741-40.63 40.63a40 40 0 0 1-43.785 8.59l-15.048-6.326M28.29-30.925-50.132 50.133a40 40 0 0 1-43.786 8.59l-11.526-4.845"/>
              </svg>
              <div style="font-family:'Space Grotesk',ui-sans-serif,system-ui,sans-serif;font-size:22px;font-weight:700;letter-spacing:-0.025em;">
                <span style="color:#4be1ec;">IR</span><span style="color:#eef2fb;">aven</span>
              </div>
            </td>
          </tr>
          ...
          <tr>
            <td bgcolor="#0b0f1d" style="background:#0b0f1d;border:1px solid rgba(150,170,220,0.18);border-radius:20px;padding:40px 40px 36px;">
              ...
              <p style="margin:0 0 14px;font-size:11.5px;font-weight:500;letter-spacing:0.26em;text-transform:uppercase;color:#2ee8c4;">
                Workspace approved
              </p>

              <h1 style="margin:0 0 14px;font-size:26px;font-weight:600;letter-spacing:-0.025em;color:#eef2fb;line-height:1.15;">
                You're in, ${firstName}.
              </h1>

              <p style="margin:0 0 20px;font-size:15px;color:#8b95ac;line-height:1.65;">
                Your IRaven Workspace request has been approved. Your playground is
                <span style="color:#c0c8da;font-weight:500;">${slug}.iraven.io</span>
                and is ready for you to sign in.
              </p>

              <!-- Slug badge -->
              <div style="background:rgba(75,225,236,0.07);border:1px solid rgba(75,225,236,0.18);border-radius:10px;padding:14px 18px;margin-bottom:28px;">
                <p style="margin:0 0 4px;font-size:11px;letter-spacing:0.18em;text-transform:uppercase;color:#4be1ec;">Your playground</p>
                <p style="margin:0;font-size:16px;font-weight:600;color:#eef2fb;">${slug}.iraven.io</p>
              </div>
              ...
              <table cellpadding="0" cellspacing="0" border="0">
                <tr>
                  <td style="border-radius:100px;background:linear-gradient(180deg,#8df0f6,#4be1ec);">
                    <a href="${loginUrl}"
                       style="display:inline-block;padding:14px 32px;font-size:15px;font-weight:600;color:#04121a;text-decoration:none;border-radius:100px;letter-spacing:-0.01em;white-space:nowrap;">
              ...
              <p style="margin:20px 0 0;font-size:12px;color:#5c6680;line-height:1.7;">
                Or visit: <a href="${loginUrl}" style="color:#4be1ec;text-decoration:none;">${loginUrl}</a>
              </p>
          ...
          <tr>
            <td align="center" style="padding-top:28px;">
              <p style="margin:0 0 8px;font-size:11px;letter-spacing:0.18em;text-transform:uppercase;color:#5c6680;">
                RAVEN://OS &middot; Workspace
              </p>
              <p style="margin:0;font-size:12px;color:#5c6680;line-height:1.6;">
                This email was sent because your workspace request was reviewed and approved by an IRaven admin.
              </p>
            </td>
          </tr>
...
export async function sendNeedsMoreInfoEmail(opts: {
  to: string
  fullName: string
  note: string
  slug: string
  ...
  if (process.env.NODE_ENV !== 'production') {
    console.log(`\n[NEEDS INFO EMAIL → ${opts.to}] note: ${opts.note}\n`)
  }
  ...
  await transporter.sendMail({
    from: `"IRaven Workspace" <${process.env.SMTP_FROM}>`,
    to: opts.to,
    subject: `More information needed — ${opts.slug} workspace request`,
    text: `Hi ${opts.fullName},\n\nThank you for your IRaven Workspace request. We need a bit more information before we can proceed.\n\nMessage from our team:\n${opts.note}\n\nPlease reply to this email with the requested information.\n\nIRaven Team`,
    html: buildSimpleHtml({
      eyebrow: 'Workspace Request',
      headline: `One more thing, ${firstName}.`,
      body: `Thank you for your interest in IRaven Workspace. We reviewed your request for <strong style="color:#c0c8da;">${opts.slug}.iraven.io</strong> and need a little more information before we can proceed.`,
      noteLabel: 'Message from our team',
      note: opts.note,
      footer: 'Please reply to this email with the requested details.',
      accentColor: '#f5c842',
    }),
  })
}
...
export async function sendRejectionEmail(opts: {
  ...
  if (process.env.NODE_ENV !== 'production') {
    console.log(`\n[REJECTION EMAIL → ${opts.to}] note: ${opts.note}\n`)
  }
  ...
  await transporter.sendMail({
    from: `"IRaven Workspace" <${process.env.SMTP_FROM}>`,
    to: opts.to,
    subject: `Your IRaven Workspace request — ${opts.slug}`,
    text: `Hi ${opts.fullName},\n\nThank you for your interest in IRaven Workspace. After reviewing your request for ${opts.slug}.iraven.io, we're unable to proceed at this time.\n\nNote from our team:\n${opts.note}\n\nIRaven Team`,
    html: buildSimpleHtml({
      eyebrow: 'Workspace Request',
      headline: `Thank you, ${firstName}.`,
      body: `We appreciate your interest in IRaven Workspace. After reviewing your request for <strong style="color:#c0c8da;">${opts.slug}.iraven.io</strong>, we&rsquo;re unable to move forward at this time.`,
      noteLabel: 'Note from our team',
      note: opts.note,
      footer: 'If you believe this is a mistake or your circumstances have changed, feel free to reach out.',
      accentColor: '#ff6b6b',
    }),
  })
}
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width,initial-scale=1.0" />
  <title>IRaven Workspace</title>
  <style>@import url('https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;500;600;700&display=swap');body{margin:0;padding:0}</style>
</head>
<body style="margin:0;padding:0;background:#050710;font-family:'Space Grotesk',ui-sans-serif,system-ui,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#050710" style="background:#050710;">
    <tr>
      <td align="center" style="padding:48px 20px 56px;">
        <table width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:480px;">
          ...
          <tr>
            <td bgcolor="#0b0f1d" style="background:#0b0f1d;border:1px solid rgba(150,170,220,0.18);border-radius:20px;padding:40px 40px 36px;">
              <p style="margin:0 0 14px;font-size:11.5px;font-weight:500;letter-spacing:0.26em;text-transform:uppercase;color:${opts.accentColor};">${opts.eyebrow}</p>
              <h1 style="margin:0 0 14px;font-size:26px;font-weight:600;letter-spacing:-0.025em;color:#eef2fb;line-height:1.15;">${opts.headline}</h1>
              <p style="margin:0 0 24px;font-size:15px;color:#8b95ac;line-height:1.65;">${opts.body}</p>
              <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-left:3px solid ${opts.accentColor};border-radius:8px;padding:16px 18px;margin-bottom:24px;">
                <p style="margin:0 0 6px;font-size:11px;letter-spacing:0.16em;text-transform:uppercase;color:${opts.accentColor};">${opts.noteLabel}</p>
                <p style="margin:0;font-size:14px;color:#d0d4e8;line-height:1.6;">${opts.note.replace(/\n/g, '<br/>')}</p>
              </div>
              <p style="margin:0;font-size:13px;color:#5c6680;line-height:1.7;">${opts.footer}</p>
            </td>
          </tr>
          <tr>
            <td align="center" style="padding-top:28px;">
              <p style="margin:0;font-size:11px;letter-spacing:0.18em;text-transform:uppercase;color:#5c6680;">RAVEN://OS &middot; Workspace</p>
            </td>
          </tr>
        </table>
      </td>
